...
$videoUri = VimeoUri::latest('created_at')->pluck('uri')->first();
$videoId = substr($videoUri, strpos($videoUri, '/') + 1); // This assumes the URI format is consistent

$videoId = preg_replace('/[^0-9]/', '', $videoId);

$vimeo = new \Vimeo\Vimeo(env('VIMEO_CLIENT_ID'), env('VIMEO_CLIENT_SECRET'), env('VIMEO_ACCESS_TOKEN'));
//dd($vimeo);
                                        ...
                                        <td class="bodytr">
                                            <div class="__btn-grp-sm flex-nowrap">
                                                ...
                                                <a href="{{route('generate-invoice',[$order->id]) }}" title="{{ translate('download_invoice') }}"
                                                    ...
                                                    <!-- Download Button -->
                                                    <a href="{{ $downloadUrl }}" style="margin-left: 10px; padding: 10px 20px; background-color: #0073e6; color: white; text-decoration: none; border-radius: 10px; display: inline-block; opacity: 1; visibility: visible;" target="_blank">Download</a>
                                                </div>
                                            </div>

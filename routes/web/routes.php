...
use App\Http\Controllers\UserVideoMessageController;


// In web.php
Route::post('/store-vimeo-uri', function () {
    $data = json_decode(request()->getContent(), true);
    $orderId = $data['order_id'] ?? null;
    $videoUri = $data['video_uri'] ?? null;

    if ($orderId && $videoUri) {
        // Store the video URI with the given order ID
        \App\Models\VimeoUri::updateOrCreate(
            ['order_id' => $orderId],
            ['uri' => $videoUri]
        );
...
Route::get('/user-video-messages', [UserVideoMessageController::class, 'index'])->name('user-video-messages.index');
Route::post('/user-video-messages', [UserVideoMessageController::class, 'store'])->name('user-video-messages.store');


Route::get('/video/{videoId}', [VimeoController::class, 'showVideo'])->name('show.video');
